...
import { generateWeeklyReport } from "@qcc/core/reports";
import { serviceWeekOf } from "@qcc/core/week";

export async function saveServiceAttendanceAction(formData: FormData) {
  ...
  await logAudit("service_attendance_recorded", leader.id, "attendance_day", dayId, {
    bacentaId,
    serviceDate,
  ...
  try {
    await generateWeeklyReport(bacentaId, serviceDate);
  } catch (err) {
    console.error("Failed to generate weekly report:", err);
  }

  revalidatePath("/attendance");
  redirect(`/attendance/service/${dayId}`);
}

export async function saveFellowshipAttendanceAction(formData: FormData) {
  const leader = await requireLeader();
  const bacentaId = String(formData.get("bacentaId") ?? "");
  ...
  const scope = await getBacentaScope(bacentaId);
  if (!scope || !canRecordAttendance(leader, scope)) {
    throw new Error("You cannot record fellowship attendance for this bacenta.");
  }

  const memberIds = formData.getAll("memberIds").map(String);
  const visitorCount = Number(formData.get("visitorCount") ?? 0) || 0;

  // Upsert the fellowship day
  ...
  if (existing) {
    ...
    await db
      ...
      .where(eq(fellowshipAttendanceDays.id, dayId));
    await db
      .delete(fellowshipAttendanceEntries)
      .where(eq(fellowshipAttendanceEntries.dayId, dayId));
  } else {
    const [row] = await db
      .insert(fellowshipAttendanceDays)
      .values({
        attendanceDate,
        bacentaId,
        takenByLeaderId: leader.id,
      ...
      .returning({ id: fellowshipAttendanceDays.id });
    dayId = row.id;
  }
  ...
  if (entries.length > 0) {
    await db.insert(fellowshipAttendanceEntries).values(entries);
  }

  await logAudit("fellowship_attendance_recorded", leader.id, "fellowship_attendance_day", dayId, {
    bacentaId,
    attendanceDate,
    entries: entries.length,
  });
  ...
  try {
    const weekOf = serviceWeekOf(new Date(attendanceDate));
    await generateWeeklyReport(bacentaId, weekOf);
  } catch (err) {
    console.error("Failed to generate weekly report:", err);
  }

  revalidatePath("/attendance");
  redirect(`/attendance/fellowship/${dayId}`);
}
...
export async function reviewAttendanceAction(formData: FormData) {
  ...
  revalidatePath("/attendance");
  revalidatePath(`/attendance/service/${dayId}`);
}
