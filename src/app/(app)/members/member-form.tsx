...
import type { ScopedBacenta } from "@/lib/scope";
    ...
    <form action={action} className="card max-w-2xl space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
          ...
          <select name="bacentaId" className="input" defaultValue={values.bacentaId ?? ""} required>
            ...
            {bacentas.map((b) => (
              <option key={b.id} value={b.id}>
                {b.name} ({b.area === "area1" ? "Area 1" : "Area 2"}) — {b.governorshipName}
              </option>
            ))}
          </select>
        </div>
        <div>
          ...
          <select name="status" className="input" defaultValue={values.status ?? "committed"}>
